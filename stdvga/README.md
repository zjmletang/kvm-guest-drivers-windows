...
For client SKUs, automation scripts, or cloud-agent contexts (Session 0), use
the helper tool built with the solution:
...
The tool works on all Windows 10+ and Windows Server 2019+ SKUs. When launched
from a non-interactive Session 0 context it automatically re-invokes itself in
the active user session, so it can be called directly from UserData scripts or
cloud-agent commands. An active interactive session must exist for the
