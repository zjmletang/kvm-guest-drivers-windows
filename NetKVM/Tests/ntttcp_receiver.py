...
LOG_FILENAME = f"perf_results_{TIMESTAMP}.log"
...
def detect_sender_platform():
    """Query sender service for its platform (Windows/Linux)."""
    try:
...
def wait_for_sender():
    ...
    while True:
        try:
            ...
            time.sleep(2)
    sender_platform = detect_sender_platform()
    ...
    for idx, (proto, size, threads) in enumerate(SCENARIOS):
        try:
            ...
            if IS_WINDOWS:
                ...
                ns_flag = "-ns" if CROSS_PLATFORM else ""
                rcv_cmd = f"{NTTTCP_CLIENT} -r {is_udp} -m {threads},*,{MY_IP} -l {recv_l} -t {TEST_TIME} -to {timeout_ms} {ns_flag} -xml {xml_file}"
            else:
                k_flag = f"--show-nic-packets {INTERFACE_NAME}" if INTERFACE_NAME else ""
                rcv_cmd = f"{NTTTCP_CLIENT} -r {is_udp} -b {recv_l} -P {threads} -N {k_flag} -x {xml_file} -t {TEST_TIME}"

            popen_kwargs = dict(shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            if not IS_WINDOWS:
                popen_kwargs['preexec_fn'] = os.setsid
            proc = subprocess.Popen(rcv_cmd, **popen_kwargs)
            time.sleep(2)
