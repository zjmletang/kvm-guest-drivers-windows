...
/* Returns the negotiated size (number of descriptors) of the queue,
 * the counterpart of the Linux virtqueue_get_vring_size().
 * The size is written once at queue construction and never changes,
 * so unlike the other queue operations this need not be serialized. */
